...
class NLocalSATLayer(nn.Module):
    """
        NLocalSAT Round from NLocalSAT.
    """
    def __init__(self, 
                 emb_size, 
                 num_fc):
        super(NLocalSATLayer, self).__init__()
    ...
    def forward(self,
        ...
        l_flip_msg = flip(l_all_msg, l_pos_index, l_neg_index)[l_index]
        _, l_state = self.L_update((cl_msg + l_flip_msg).unsqueeze(0), l_state)
