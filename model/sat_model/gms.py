...
class GMS(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_l_msg_mlp = MLP(self.hidden_size, self.hidden_size, self.hidden_size, num_layer=self.num_fc)
        self.neg_l_msg_mlp = MLP(self.hidden_size, self.hidden_size, self.hidden_size, num_layer=self.num_fc)
        self.pos_c_msg_mlp = MLP(self.hidden_size, self.hidden_size, self.hidden_size, num_layer=self.num_fc)
        self.neg_c_msg_mlp = MLP(self.hidden_size, self.hidden_size, self.hidden_size, num_layer=self.num_fc)
        self.l_update = nn.LSTM(self.hidden_size, self.hidden_size)
    ...
    def forward(self, lcg_graph, l_embedding, c_embedding):
        ...
        for round_idx in enumerate(range(self.num_round)):
            ...
            pos_c2l_msg = self.conv(lcg_graph, "c", "pos_c2l", "pos_l", pos_c_msg)
            neg_c2l_msg = self.conv(lcg_graph, "c", "neg_c2l", "neg_l", neg_c_msg)
